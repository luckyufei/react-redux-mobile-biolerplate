 ...
 */
export function convertDateType(dateType, startDate, endDate) {
  let result = { startDate: '', endDate: '', len: 0 };
  const TODAY = moment();
  let start = TODAY, end = TODAY, len = 0;
  ...
  switch (dateType) {
    ...
    case 'last_1_month':
      ...
      break;
    case 'custom':
      start = startDate ? moment(startDate) : TODAY;
      end = endDate ? moment(endDate) : TODAY;
      len = end.diff(start, 'days');
      break;
  }
  return { startDate: `${start.format(YYYY_MM_DD)} 00:00:00`, endDate: `${end.format(YYYY_MM_DD)} 23:59:59`, len };
}


export function getDateStr(timestamp, format) {
  return moment(timestamp ? new Date(timestamp) : new Date()).format(format || YYYY_MM_DD);
}
